...
**Дополнительно:** реализована возможность выполнения трёх попыток повторного запроса, если запрос был неудачным с помощью доп. опции retry в RTK Query (src/api/api.ts)
